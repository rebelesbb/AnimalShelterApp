...
public class AuthServiceImpl implements AuthService {
    ...
    public User loginAsUser(LoginRequest request) {
        Optional<User> userOpt = userRepository.findByUsernameAndPassword(request.getUsername(), hashPassword(request.getPassword()));
        return userOpt.orElse(null);
    }

    @Override
    public Employee loginAsEmployee(LoginRequest request) {
        Optional<Employee> empOpt = employeeRepository.findByUsernameAndPassword(request.getUsername(), request.getPassword());
